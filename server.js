...
// Get products available for distributor's warehouse
app.get("/api/distributor/products", async (req, res) => {
  const { distributorId } = req.query;
  if (!distributorId) return res.status(400).json({ error: "Missing distributorId" });

  try {
    // Get distributor's warehouse name
    const [distributor] = await connection.query(
      "SELECT warehouse FROM distributors WHERE id = ?",
      [distributorId]
    );
    if (distributor.length === 0) return res.status(404).json({ error: "Distributor not found" });

    const warehouseName = distributor[0].warehouse;

    // Get products from that warehouse
    const [products] = await connection.query(
      "SELECT id, name FROM products WHERE warehouse = ?",
      [warehouseName]
    );
...
app.post("/api/distributor/orders", async (req, res) => {
  ...
  if (!distributorId || !productId || !qty) return res.status(400).json({ error: "Missing fields" });
